...
// Configuration
const portIPv6 = 8845; // Port for the Mobility Sensor
...
serverUDP.on('listening', function () {
  ...
  console.log(`[UDP - IPv6] Mobility Border Router listening at ${address.address}:${address.port}`);
});
...
function processMessage(message, remote) {
  ...
  keyValuePairs.forEach((pair) => {
    ...
    if (key === 'location' || key === 'geo_fence' || key === 'id') {
      dataBlock[key] = value;
    } else {
