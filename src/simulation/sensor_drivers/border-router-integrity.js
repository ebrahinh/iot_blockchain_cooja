...
// Configuration
const portIPv6 = 8843; // Port for the Integrity Sensor
...
serverUDP.on('listening', function () {
  ...
  console.log(`[UDP - IPv6] Integrity Border Router listening at ${address.address}:${address.port}`);
});
...
function processMessage(message, remote) {
  ...
  keyValuePairs.forEach((pair) => {
    ...
    if (key === 'integrity' || key === 'auth_status' || key === 'id') {
      dataBlock[key] = value;
    } else {
