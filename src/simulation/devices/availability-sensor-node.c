...
#define LOG_MODULE "Availability-Sensor"
...
void getAvailabilityFlag(int *data) {
    *data = rand() % 2; // Random 0 (unavailable) or 1 (available)
}

void getUsageAnomaly(int *data) {
    *data = rand() % 2; // Random 0 (normal) or 1 (anomaly detected)
}

void getEncryptionStrength(int *data) {
    *data = rand() % 101; // Random percentage between 0 and 100%
}

void getTamperFlag(int *data) {
    *data = rand() % 2; // Random 0 (no tampering) or 1 (tampering detected)
}
...
PROCESS_THREAD(init_system_proc, ev, data) {
    ...
    while (1) {
        ...
        int availability_flag, usage_anomaly, enc_strength, tamper_flag;
        getAvailabilityFlag(&availability_flag);
        getUsageAnomaly(&usage_anomaly);
        getEncryptionStrength(&enc_strength);
        getTamperFlag(&tamper_flag);
