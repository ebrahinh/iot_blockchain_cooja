...
void getAuthStatus(int *data) {
    *data = rand() % 2; // Random 0 (unauthenticated) or 1 (authenticated)
}
...
void getEncTime(int *data) {
    *data = rand() % 100 + 10; // Random encryption time between 10–109 ms
}
...
PROCESS_THREAD(init_system_proc, ev, data) {
    ...
    while (1) {
        ...
        int auth_status, integrity_flag, threat_flag, enc_time;
        ...
        snprintf(buff_udp, sizeof(buff_udp),
                 "auth_status:%d,integrity_flag:%d,threat_flag:%d,enc_time:%d",
                 auth_status, integrity_flag, threat_flag, enc_time);
