...
: "${CONNECTION_PROFILE_FILE_SENSORS:=${TEST_NETWORK_HOME}/organizations/peerOrganizations/sensors.example.com/connection-sensors.json}"
: "${CERTIFICATE_FILE_SENSORS:=${TEST_NETWORK_HOME}/organizations/peerOrganizations/sensors.example.com/users/User1@sensors.example.com/msp/signcerts/User1@sensors.example.com-cert.pem}"
: "${PRIVATE_KEY_FILE_SENSORS:=${TEST_NETWORK_HOME}/organizations/peerOrganizations/sensors.example.com/users/User1@sensors.example.com/msp/keystore/priv_sk}"
...
HLF_CERTIFICATE_SENSORS="$(cat ${CERTIFICATE_FILE_SENSORS} | sed -e 's/$/\\n/' | tr -d '\r\n')"

HLF_PRIVATE_KEY_SENSORS="$(cat ${PRIVATE_KEY_FILE_SENSORS} | sed -e 's/$/\\n/' | tr -d '\r\n')"
...
SENSORS_APIKEY=$(uuidgen)

ENV_END

# Configure local or remote host settings
...
HLF_CONNECTION_PROFILE_SENSORS=$(cat ${CONNECTION_PROFILE_FILE_SENSORS} | jq -c .)
...
HLF_CONNECTION_PROFILE_SENSORS=$(cat ${CONNECTION_PROFILE_FILE_SENSORS} | jq -c '.peers["peer0.sensors.example.com"].url = "grpcs://peer0.sensors.example.com:7051" | .certificateAuthorities["ca.sensors.example.com"].url = "https://ca.sensors.example.com:7054"')
