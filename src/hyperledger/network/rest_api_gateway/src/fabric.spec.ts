...
describe('Fabric', () => {
  describe('createWallet', () => {
    it('creates a wallet containing identities for all organizations', async () => {
      const wallet = await createWallet();

      expect(await wallet.list()).toStrictEqual([
        'SecurityOrgMSP',
        'IntegrityOrgMSP',
        'NetworkMobilityOrgMSP',
        'AvailabilitySensorMSP',
      ]);
    });
  });
  ...
  describe('createGateway', () => {
    it('creates a Gateway and connects using the provided arguments', async () => {
      const connectionProfile = config.connectionProfileSecurityOrg;
      const identity = config.mspIdSecurityOrg;
  ...
  describe('getContracts', () => {
    it('gets the asset and qscc contracts from the network', async () => {
      ...
      const contracts = await getContracts(mockNetwork, config.chaincodeName);
  ...
  describe('evatuateTransaction', () => {
    ...
    it('throws an AssetExistsError if asset already exists', async () => {
    ...
    it('throws an AssetNotFoundError if asset does not exist', async () => {
      mockTransaction.evaluate.mockRejectedValue(
  ...
  describe('submitTransaction', () => {
    ...
    it('throws an AssetExistsError if asset already exists', async () => {
      ...
      await expect(async () => {
        await submitTransaction(mockTransaction, 'txn', 'arga', 'argb');
      }).rejects.toThrow(AssetExistsError);
    });
  });
